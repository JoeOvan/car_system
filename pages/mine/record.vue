		  ...
		  <swiper-item class="swiper-item" v-for="(item, index) in list" :key="index">
			...
			<block v-if="item.data && item.data.length >= 1">
				...
				<block v-for="(item1, index1) in item.data" :key="index1">
					...
					<view class="order-list" @click="toPage(item1.workDate, item1.orderSn)" :url="'/pages/mine/record-detail?orderSn='+item1.orderSn + '&title='+item1.orderName">
						<view class="order-info">
							<image class="icon-order" src="../../static/images/icon-order.png"></image>
							<text class="date">{{item1.workDate}}&nbsp;工单</text>
						</view>
...
<script>
	export default {
		...
		methods: {
			...
			toPage(workDate,orderSn) {
				
				uni.navigateTo({
					url: '/pages/mine/record-detail?workDate=' + workDate + '&orderSn=' + orderSn
				})
