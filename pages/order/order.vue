					...
					<block v-if="item.data && item.data.length >= 1">
						...
						<block v-for="(item1, index1) in item.data" :key="index1">
							...
							<block v-if="item1.orderState == 1">
								<view class="order-list" @click="toPage(item1.workDate, item1.orderSn)">
	...
	export default {
		...
		methods: {
			...
			toPage(workDate,orderSn) {
				...
				uni.navigateTo({
					url: '/pages/order/order-detail?workDate=' + workDate + '&orderSn=' + orderSn
				})
